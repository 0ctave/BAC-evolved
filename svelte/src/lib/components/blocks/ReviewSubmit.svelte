<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			...
			console.error('Failed to validate UUID', _e);
			isValid = false;
		} finally {
			isValidating = false;
		}
	...
	const handleSubmit = async (e: Event) => {
		...
		if (content.length < 10) {
			errorMessage =
				d.msg_error_min_content || 'Votre commentaire doit faire au moins 10 caractères.';
			return;
		}
...
<div class="mx-auto max-w-2xl px-6 py-12 md:py-20">
	...
	{:else if !isValid}
		<div class="surface-atelier p-8 text-center" in:fade>
			...
			<a href="/" class="btn-atelier-outline mt-8 inline-block"
				>{d.button_home || "Retour à l'accueil"}</a
			>
		</div>
		...
		<div class="surface-atelier p-8 text-center md:p-12" in:fly={{ y: 20, duration: 600 }}>
			<div class="mb-6 flex justify-center">
				<div class="bg-primary/10 rounded-full p-4">
					<svg class="text-primary h-12 w-12" fill="none" viewBox="0 0 24 24" stroke="currentColor">
						<path
			...
			<h2 class="heading-section mb-4">
				{isSuccess ? d.msg_success_title || 'Merci !' : d.msg_already_submitted || 'Merci !'}
				{clientName}
			</h2>
			<p class="text-body mb-8">
				...
				{:else}
					{d.msg_already_submitted_desc ||
						'Votre avis a déjà été enregistré. Merci de votre retour !'}
				{/if}
			</p>
	...
	{:else}
		<div class="mb-10 text-center">
			...
			{/if}
			<p class="text-body mt-4">
				{(d.intro_text || "Bonjour {name}, comment s'est déroulé votre séjour ?").replace(
					'{name}',
					clientName
				)}
			</p>
		...
		<div class="surface-atelier p-8 md:p-12" in:fly={{ y: 30, duration: 800 }}>
			<form onsubmit={handleSubmit} class="space-y-8">
				...
				<div class="space-y-3">
					<label
						class="font-heading text-iron dark:text-limestone-50 block text-lg font-bold"
						for="pseudonyme"
					>
					...
					{#if d.help_pseudonyme}
						<p class="text-iron-muted text-xs">{d.help_pseudonyme}</p>
					{/if}
				</div>
