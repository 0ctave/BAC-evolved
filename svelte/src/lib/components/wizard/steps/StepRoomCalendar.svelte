<script lang="ts">
	...
	$effect(() => {
		if (booking.validationTrigger > 0) showValidation = true;
	});
	$effect(() => {
		if (booking.resetTrigger > 0) showValidation = false;
	});
	...
	const roomPrices = $derived.by(() => {
		...
		if (value.start && value.end) {
			...
			if (h.compare(s) >= 0) {
				simStart = s;
				simEnd = h;
				if (simEnd.compare(simStart) === 0) simEnd = simStart.add({ days: 1 });
			} else {
				simStart = h;
				simEnd = s;
			}
			...
			if (isSimulatedBestPrice) {
				...
				durationsToTest.forEach((nights) => {
					...
					const result = calculateStayPrice(
						...
						testEnd,
						pricingRules,
						{ adults: adults, children: children, parking: false, roomId: room.id }
					);
				...
				const result = calculateStayPrice(Number(room.prix_nuit), simStart, simEnd, pricingRules, {
					adults: adults,
					children: children,
					parking: false,
					roomId: room.id
	...
	let isRoomSelected = $derived(
		!!booking.roomSelection.chambre && booking.roomSelection.chambre !== 'any'
	);
	let isDateInvalid = $derived(showValidation && !!booking.errors.room.dates);
	let isRoomInvalid = $derived(
		showValidation && !booking.errors.room.dates && !!booking.errors.room.chambre
	);
		...
		if (selectedRoomId !== 'any') {
			...
			if (validRooms.length > 0) {
				...
				if (barriers.some((b) => b.rMax === null)) {
					...
					max = barriers.reduce(
						(acc, curr) => (!acc || (curr.rMax && curr.rMax.compare(acc) > 0) ? curr.rMax : acc),
				...
				if (barriers.some((b) => b.rMin === null)) {
					...
					min = barriers.reduce(
						(acc, curr) => (!acc || (curr.rMin && curr.rMin.compare(acc) < 0) ? curr.rMin : acc),
						null as DateValue | null
					);
				}
			} else {
	...
	$effect(() => {
		...
		booking.roomSelection.date_arrivee = value.start
			? (value.start as unknown as CalendarDate)
			: null;
	...
	const cellClasses = $derived.by(() => {
		...
		for (let i = 0; i < 42; i++) {
			...
			const isSelected =
				start && end && currentDate.compare(start) >= 0 && currentDate.compare(end) <= 0;
			...
			if (start && !end && barriers.max && isSameDay(currentDate, barriers.max)) {
				isCheckoutCandidate = true;
			}
			if (start && end && isSameDay(currentDate, end)) {
				isCheckoutCandidate = true;
			}
			...
			if (isRestricted) {
				...
				assignedClass = 'cal-day-blocked';
			} else if (isSelected || isHoverRange) {
				if (visualFull) {
					assignedClass = 'cal-day-invalid-selection';
				} else {
					assignedClass = 'cal-day-selected-range';
				}
			} else {
				if (visualFull) {
					assignedClass = 'cal-day-booked';
				} else if (selectedRoomId === 'any' && rooms.length >= 2) {
	...
	<div class="space-y-8">
		...
		{#if isDateInvalid}
			<div
				...
				class="flex items-center gap-3 rounded-r-md border-l-4 border-red-500 bg-red-50 p-4 shadow-sm dark:bg-red-900/10"
			>
				<svg
		...
		<TravelersSelector bind:adults bind:children {maxAdults} {maxChildren} {totalCapacity} />

		<OptionSelector bind:parking={booking.roomSelection.parking} {parkingDetails} />
	</div>
	...
	<div class="flex h-full flex-col space-y-6">
		<div>
			<span class="text-primary text-xs font-bold tracking-widest uppercase"
				>{l.step_calendar_step_label}</span
			>
			<h3 class="text-iron dark:text-limestone-50 text-xl font-bold">{l.step_calendar_title}</h3>
		</div>
