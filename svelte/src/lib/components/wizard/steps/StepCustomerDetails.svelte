...
<div class="mx-auto max-w-2xl px-4 sm:px-6">
	...
	<div class="mb-10 text-center" in:fly={{ y: -20, duration: 500 }}>
		...
		<p class="text-body mt-2 text-center italic">
			{l.customer_desc}
		</p>
	</div>
	...
	<form class="space-y-6" onsubmit={handleSubmit}>
		<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
			...
			<div class="group" in:fly={{ y: 20, duration: 500, delay: 100 }}>
				<label
					for="prenom"
					class="text-iron/80 dark:text-limestone-300 group-focus-within:text-primary mb-2 block text-sm font-medium transition-colors duration-300"
				>
				...
				<div class="relative">
					...
					/>
					{#if showError('prenom')}
						<div transition:slide={{ duration: 200 }} class="mt-1 text-xs font-medium text-red-500">
		...
		<div class="group" in:fly={{ y: 20, duration: 500, delay: 250 }}>
			...
			<div class="relative w-full">
				<div
					class="flex w-full items-stretch overflow-hidden border-b-2 transition-all
                    duration-200
					{showError('telephone')
						? 'border-red-500 bg-red-50/10'
						: 'focus-within:!border-primary focus-within:bg-limestone-50/50 dark:focus-within:bg-iron-light/20 border-black/20 hover:border-black/40 dark:border-white/20 dark:hover:border-white/40'}"
				>
					<div class="relative flex w-[90px] flex-shrink-0 cursor-pointer items-center">
						<div
							class="text-iron dark:text-limestone-50 flex w-full items-center justify-between px-2 py-3"
						>
							<div class="flex items-center gap-2">
								{#if selectedCountry}
									<span
										class="fi fi-{selectedCountry.toLowerCase()} !h-[18px] !w-6 flex-shrink-0 rounded-sm !bg-cover shadow-sm"
									></span>
									<span class="font-mono text-xs font-bold opacity-80">{selectedCountry}</span>
								{/if}
							</div>
							<svg
								class="text-iron/40 dark:text-limestone-400/40 h-3 w-3"
								fill="none"
								stroke="currentColor"
								viewBox="0 0 24 24"
								><path
					...
					<div class="my-2 w-[1px] bg-black/10 dark:bg-white/10"></div>
					...
					<div class="relative flex-grow">
						<TelInput
							...
							class="placeholder-iron-300 text-iron dark:text-limestone-50 h-full w-full border-none bg-transparent px-3 py-3 font-medium outline-none focus:ring-0"
						/>
		...
		<div class="group relative" in:fly={{ y: 20, duration: 500, delay: 300 }}>
			...
			<div class="relative">
				<select
					...
					class="input-atelier w-full cursor-pointer appearance-none pr-10"
				>
				...
				<div
					class="text-iron-muted dark:text-limestone-400 pointer-events-none absolute inset-y-0 right-0 flex items-center px-3"
				>
					<svg
						class="h-4 w-4 fill-current transition-transform group-focus-within:rotate-180"
